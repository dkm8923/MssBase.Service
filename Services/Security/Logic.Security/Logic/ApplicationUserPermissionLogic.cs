...
namespace Logic.Security.Logic
{
    public class ApplicationUserPermissionLogic : IApplicationUserPermissionLogic
    {
        ...
        {
            var res = await this.Filter(new FilterApplicationUserPermissionLogicRequest { ApplicationUserPermissionIds = new List<int> { applicationUserPermissionId }, IncludeInactive = req.IncludeInactive, IncludeRelated = req.IncludeRelated, IncludeReadOnly = req.IncludeReadOnly, CurrentUser = req.CurrentUser });
        ...
        /// </summary>
        public async Task<ErrorValidationResult<IEnumerable<ApplicationUserPermissionDto>>> Filter(FilterApplicationUserPermissionLogicRequest req, CancellationToken cancellationToken = default)
        {
            ...
            using (var dbContext = _dbContextFactory.CreateContextReadOnly())
            {
                ...
                return new ErrorValidationResult<IEnumerable<ApplicationUserPermissionDto>> { Response = await query.ToDtos(cancellationToken) };
            }
        }
