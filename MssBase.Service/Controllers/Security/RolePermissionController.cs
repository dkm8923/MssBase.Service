...
namespace MssBase.Service.Controllers.Security
{
    ...
    public class RolePermissionController : ApiBaseController
    {
        ...
        public async Task<IActionResult> GetRolePermissions([FromQuery] bool deleteCache = false, [FromQuery] bool includeInactive = false, [FromQuery] bool includeRelated = false, CancellationToken cancellationToken = default)
        {
            try
            {
                var records = await _rolePermissionSvc.GetAll(new BaseServiceGet { DeleteCache = deleteCache, IncludeInactive = includeInactive, IncludeRelated = includeRelated }, cancellationToken);
                return Ok(records);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetRolePermissionById(int applicationUserId, [FromQuery] bool deleteCache = false, [FromQuery] bool includeInactive = false, [FromQuery] bool includeRelated = false, CancellationToken cancellationToken = default)
        {
            try
            {
                var record = await _rolePermissionSvc.GetById(applicationUserId, new BaseServiceGet { DeleteCache = deleteCache, IncludeInactive = includeInactive, IncludeRelated = includeRelated }, cancellationToken);
